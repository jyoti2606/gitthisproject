...
it("test status codes", async () => {
   const { data } = await GET` /escalation-management/Statuses?$select=code`;
        ...
        it("test action resolve", async () => {            
                ...
                `/escalation-management/Escalations(ID=${draft.ID},IsActiveEntity=false)/
                EscalationManagementService.draftActivate`
                );
                ...
                await POST`/escalation-management/Escalations(ID=${draft.ID},IsActiveEntity=false)/
                EscalationManagementService.resolve`;
                // Step 6: Read the escalation after executing the resolve action
                let {
                data: readAfterAction,
                } = await GET`/escalation-management/Escalations(ID=${post.ID},IsActiveEntity=true)`;
                // Step 7: Check if the escalation is updated to the status 'CMP' -
                Completed
